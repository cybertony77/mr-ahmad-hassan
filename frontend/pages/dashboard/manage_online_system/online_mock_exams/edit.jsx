...
export default function EditMockExam() {
  ...
  useEffect(() => {
    if (mockExamData && !dataLoaded && !dataLoadedRef.current) {
      ...
      setFormData({
        ...
        questions: mockExamData.questions && Array.isArray(mockExamData.questions)
      ...
      const loadImageUrls = async () => {
        const urlPromises = mockExamData.questions?.map(async (q, idx) => {
          if (q.question_picture) {
            setLoadingImages(prev => ({ ...prev, [idx]: true }));
            try {
              const response = await apiClient.get(`/api/online_mock_exams/image?public_id=${q.question_picture}`);
  ...
  const handleSubmit = (e) => {
    ...
    const submitData = {
      ...
      questions: formData.questions && Array.isArray(formData.questions) ? formData.questions.map(q => ({
        question_text: q.question_text || '',
